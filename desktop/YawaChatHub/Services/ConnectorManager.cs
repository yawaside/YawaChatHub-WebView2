...
{
    // Событие требуется интерфейсом, но заглушка не меняет состояние:
    // статус Unsupported выставляет сам ConnectorManager.
...
public sealed class YoutubeConnector : IConnector
{
    ...
    private static IEnumerable<JsonObject> FindByKey(JsonNode? node, string key)
    {
        if (node is JsonObject obj)
        {
            ...
            {
                foreach (var n in FindByKey(pair.Value, key)) yield return n;
            }
        }
    ...
    private static void CollectTokens(JsonNode? node, List<string> acc)
    {
        if (node is JsonObject obj)
        {
            ...
            foreach (var pair in obj) CollectTokens(pair.Value, acc);
        }
        else if (node is JsonArray array)
        {
