...
public sealed class ConnectorManager
{
    ...
    public void Diagnose()
    {
        foreach (var ch in List())
            BridgeHost.Current?.EmitChat(ChatMsg.Create(ch.Platform, ch.ChannelId, "system", $"диагностика: канал {ch.ChannelId} активен"));
    }
...
public sealed class StubConnector : IConnector
{
    ...
    public async Task Run()
    {
        ...
        BridgeHost.Current?.EmitChat(ChatMsg.Create(_platform, _channel, "system",
        ...
        while (true)
        {
            try
            {
                ...
                while ((line = await reader.ReadLineAsync()) != null)
                {
                    ...
                    BridgeHost.Current?.EmitChat(ChatMsg.Create(_platform, _channel, author, text));
                }
            }
