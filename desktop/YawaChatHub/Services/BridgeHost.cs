...
{
    // Каждый WebView2 (главное окно + оверлей) создаёт свой мост. Чтобы
    // сообщения из коннекторов доходили и в ленту, и в оверлей, все мосты
    // регистрируются в общем списке, а Emit рассылает событие каждому.
    private static readonly object InstLock = new();
    private static readonly List<BridgeHost> Instances = new();

    public BridgeHost()
    {
        lock (InstLock) Instances.Add(this);
    ...
    /// <summary>Текущий мост главного окна — для сервисов, работающих с UI-контекстом.</summary>
    ...
    /// <summary>Рассылaет событие всем активным окнам (главное + оверлей).</summary>
    private void Emit(string eventName, string detail)
    {
        var js = $"window.dispatchEvent(new CustomEvent('{eventName}', {{ detail: {detail} }}));";
        List<BridgeHost> snapshot;
        lock (InstLock) snapshot = Instances.ToList();
        foreach (var bridge in snapshot)
        {
            var script = bridge.Script;
    ...
    {
        // Синхронизацию настроек выполняет только главный мост (MainWindow).
        // Оверлей подписывается на SettingsService.Changed напрямую в OverlayWindow.
