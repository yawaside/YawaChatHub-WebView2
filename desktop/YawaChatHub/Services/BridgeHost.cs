...
{
    private static readonly object BridgesLock = new();
    private static readonly List<WeakReference<BridgeHost>> Bridges = new();

    public BridgeHost()
    {
        lock (BridgesLock) Bridges.Add(new WeakReference<BridgeHost>(this));
    ...
    private static void EmitAll(string eventName, string detail)
    {
        var alive = new List<BridgeHost>();
        lock (BridgesLock)
        {
            for (var i = Bridges.Count - 1; i >= 0; i--)
            {
                if (Bridges[i].TryGetTarget(out var bridge)) alive.Add(bridge);
                else Bridges.RemoveAt(i);
            }
        }
        foreach (var bridge in alive) bridge.Emit(eventName, detail);
    }
    ...
    private void OnSettingsChanged(Settings settings)
    {
        ...
        EmitAll("sp:settings", SettingsService.Instance.ToJson());
    ...
    {
        // Ровно одна рассылка каждому живому WebView: main + overlay.
        // OBS получает ту же модель отдельным каналом WidgetServer.
        try { WidgetServer.Instance.Broadcast(new { type = "chat", msg }); } catch { }
        EmitAll("sp:chat", Json(msg));
    }

    public void EmitChannelStatus(string platform, string channel, ChannelState state)
        => Emit("sp:channel:status",
            Json(new { platform, channel, status = state.ToString().ToLowerInvariant() }));
...
public static class OverlayManager
{
    ...
    {
        var next = !SettingsService.Instance.Current.Overlay.Enabled;
        SettingsService.Instance.Patch(
            $"{{\"overlay\":{{\"enabled\":{next.ToString().ToLowerInvariant()}}}}}");
    }

    public static void Apply(OverlayConfig cfg)
    {
        if (_win == null && cfg.Enabled) _win = new OverlayWindow();
        if (_win == null) return;
