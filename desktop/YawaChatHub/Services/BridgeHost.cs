...
public class BridgeHost
{
    ...
    // ── Приложение ───────────────────────────────────────────────────────────
    public void appQuit() => UI(() => System.Windows.Application.Current.Shutdown());

    // ── Внутренние эмиттеры (вызываются сервисами) ───────────────────────────
    public void EmitChat(ChatMsg msg) => Emit("sp:chat", Json(msg));
    public void EmitChannels(List<Channel> channels) => Emit("sp:channels", Json(channels));
    public void EmitSettings(Settings s) => Emit("sp:settings", Json(s));
...
public static class OverlayManager
{
    ...
    {
        if (_win == null) _win = new OverlayWindow();
        _win.Toggle();
    }
    ...
    public static void Apply(OverlayConfig cfg)
    {
        ...
        if (_win != null) _win.ApplyConfig(cfg);
