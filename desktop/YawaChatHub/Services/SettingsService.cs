...
public sealed class SettingsService
{
    ...
    // Автосохранение (ТЗ §14.2) вызывается на каждое движение ползунка, поэтому
    // запись на диск вынесена в фон и «схлопывается»: UI-поток больше не ждёт I/O.
    ...
    private void SaveNow()
    {
        ...
        {
            string json;
            lock (_lock) json = JsonSerializer.Serialize(Current, _json);
