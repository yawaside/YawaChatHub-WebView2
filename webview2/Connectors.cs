...
public sealed class ConnectorManager : IDisposable
{
    ...
    public void Connect(ChannelSubInput sub)
    {
        ...
        // статус уходит в интерфейс только при фактическом изменении:
        // коннекторы шлют статус каждые 15–45 секунд, и каждый такой повтор
        // вызывал перерисовку ленты и оверлея без нужды
        var lastStatus = "";
        var lastViewers = -1;
        void status(string s, int v)
        {
            if (s == lastStatus && v == lastViewers) return;
            lastStatus = s;
            lastViewers = v;
...
public sealed class TikTokConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // ОСНОВНОЙ путь — скрытое окно WebView2 со страницей эфира: браузер
        // сам получает cookies и подпись запросов, поэтому пользователю
        // достаточно указать имя канала.
        var reader = ConnectorManager.TikTokReader;
        if (reader != null)
        {
            // счётчик зрителей приходит отдельным событием состояния;
            // не сбрасываем его в 0 при каждом новом сообщении
            var live = false;
            var lastViewers = 0;
            await reader(
                _user,
        ...
        // Запасной путь (без окна приложения): публичный webcast-API.
