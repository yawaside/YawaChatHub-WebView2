...
public sealed class TikTokConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // roomId, в отличие от парсинга HTML страницы /live, который отдавал
        // заглушку и эфир никогда не находился. Клиент — с cookies.
            ...
            if (payload.Length == 0)
            {
                ...
                if (++emptyStreak == 40 && !warned)
                {
                    ...
                    _emit(new ChatEvent
                    {
                        ...
                        channel = _user,
                        kind = "system",
                        text = "TikTok: эфир идёт, но чат не отдаётся. Добавьте канал заново и вставьте cookie из браузера (поле токена).",
                    });
                }
