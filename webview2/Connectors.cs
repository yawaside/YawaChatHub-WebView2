...
public sealed class TwitchIrcConnector : IPlatformConnector
{
    ...
    {
        TwitchIrcConnector? conn;
        lock (Live) Live.TryGetValue(channel.Trim().TrimStart('#', '@'), out conn);
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        _writer = writer;
        _authenticated = !string.IsNullOrWhiteSpace(_botToken) && !string.IsNullOrWhiteSpace(_botLogin);
        ...
        if (!string.IsNullOrWhiteSpace(_botToken) && !string.IsNullOrWhiteSpace(_botLogin))
        {
            await writer.WriteLineAsync($"PASS oauth:{_botToken}");
            await writer.WriteLineAsync($"NICK {_botLogin.ToLowerInvariant()}");
        }
        else
        {
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    try { await Task.Delay(TimeSpan.FromSeconds(60), viewersCts.Token); } catch { return; }
                    try
                    {
                        var page = await http.GetStringAsync(liveUrl, viewersCts.Token);
                    ...
                    if (lcc.TryGetProperty("actions", out var actions))
                        foreach (var a in actions.EnumerateArray())
                        {
                            ...
                            if (m.TryGetProperty("message", out var msg) && msg.TryGetProperty("runs", out var runs))
                                ...
                                {
                                    if (r2.TryGetProperty("text", out var tEl)) { sb.Append(tEl.GetString()); continue; }
                                    if (!r2.TryGetProperty("emoji", out var emo)) continue;

                                    // Стандартные эмодзи Unicode приходят с emojiId вида "😀";
                                    // кастомные смайлы канала — картинкой в thumbnails.
                                    // Раньше всегда подставлялся текстовый shortcut (:smile:),
                                    // поэтому вместо смайлов был текст.
                                    var isCustom = emo.TryGetProperty("isCustomEmoji", out var ice)
                                                   && ice.ValueKind == JsonValueKind.True;
                                    var emojiUrl = "";
                                    if (Net.TryFind(emo, "thumbnails", out var thumbs) && thumbs.ValueKind == JsonValueKind.Array
                                    ...
                                    if (isCustom && emojiUrl.Length > 0)
                                        ...
                                        sb.Append(emojiId);          // обычный Unicode-смайл
...
public sealed class TikTokConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // Поиск эфира: официальный live-эндпоинт TikTok. Он отдаёт статус и
        // roomId без cookies, в отличие от парсинга HTML страницы /live,
        // который отдавал заглушку и эфир никогда не находился.
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                // status: 2 = идёт эфир, 4 = офлайн
                var liveStatus = Net.FindInt(root, "status");
                var id = Net.FindString(root, "roomId") ?? "";
                if (id.Length == 0 && Net.TryFind(root, "roomId", out var rid) && rid.ValueKind == JsonValueKind.Number)
                    id = rid.GetRawText();
                viewers = Net.FindInt(root, "userCount");

                if (liveStatus == 2 && id.Length > 0)
                {
                    roomId = id;
                    break;
                }

                // Канал существует, но эфира нет — это норма, ждём начала.
                _status(Net.TryFind(root, "user", out _) ? "connected" : "error", 0);
            }
            catch
            {
                _status("connected", 0);
            ...
            if (doc.RootElement.TryGetProperty("data", out var d))
            {
                ...
                if (d.TryGetProperty("messages", out var msgs) && msgs.ValueKind == JsonValueKind.Array)
                    foreach (var m in msgs.EnumerateArray())
                    {
                        ...
                        // TikTok присылает картиночные эмоции отдельно: сам текст
                        // содержит плейсхолдеры вида [emoji_1], а картинки лежат
                        // в emotes/emote_list. Подставляем их как <img>.
                        content = InlineTikTokEmotes(content, m);
                        if (content.Length == 0) continue;
    ...
    /// <summary>
    /// Картиночные эмоции TikTok. В webcast они приходят отдельным списком
    /// (emotes / emote_list) с позицией внутри текста; без подстановки в
    /// чате оставались плейсхолдеры или пустые места вместо смайлов.
    /// </summary>
    private static string InlineTikTokEmotes(string content, JsonElement m)
    {
        if (!Net.TryFind(m, "emotes", out var list) || list.ValueKind != JsonValueKind.Array)
            if (!Net.TryFind(m, "emote_list", out list) || list.ValueKind != JsonValueKind.Array)
                return content;

        // собираем (позиция, картинка) и вставляем с конца, чтобы не сбить индексы
        var items = new List<(int pos, string url)>();
        foreach (var e in list.EnumerateArray())
        {
            var url = Net.FindString(e, "url_list") ?? "";
            if (url.Length == 0 && Net.TryFind(e, "url_list", out var urls) && urls.ValueKind == JsonValueKind.Array
                && urls.GetArrayLength() > 0)
                url = urls[0].GetString() ?? "";
            if (url.Length == 0) url = Net.FindString(e, "url") ?? "";
            if (url.Length == 0) continue;
            items.Add((Net.FindInt(e, "place_in_content"), url));
        }
        if (items.Count == 0) return content;

        items.Sort((a, b) => b.pos.CompareTo(a.pos));
        var cps = Net.ToCodePoints(content);
        foreach (var (pos, url) in items)
        {
            var token = Net.Emote(url, "emote");
            if (pos >= 0 && pos <= cps.Count) cps.Insert(pos, token);
            else cps.Add(token);
        }
        return string.Concat(cps);
    }
