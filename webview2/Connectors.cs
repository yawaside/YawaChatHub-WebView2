...
public sealed class TikTokConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // СНАЧАЛА лёгкий API-запрос. Скрытый Chromium запускается только
        // когда эфир действительно идёт — офлайн-канал больше не держит
        // 150–300 МБ RAM и фоновые JS-процессы.
        string roomId = "";
        int viewers = 0;
        using (var probe = Net.Http())
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    roomId = Net.FindString(root, "roomId") ?? "";
                    if (roomId.Length == 0 && Net.TryFind(root, "roomId", out var rid)
                        && rid.ValueKind == JsonValueKind.Number) roomId = rid.GetRawText();
                    viewers = Net.FindInt(root, "userCount");

                    if (liveStatus == 2 && roomId.Length > 0) break;
                    _status(Net.TryFind(root, "user", out _) ? "connected" : "error", 0);
                }
                catch { _status("connected", 0); }
            ...
            await reader(
                ...
                (author, text) =>
                {
                    ...
                    _emit(new ChatEvent
                    {
                        ...
                        author = author, text = text, kind = "chat",
                    });
                },
                (_, count) =>
                {
                    // Эфир уже подтверждён live-API. DOM может не показать
                    // video (мы блокируем media), поэтому не понижаем статус.
                    live = true;
