...
public sealed class TwitchIrcConnector : IPlatformConnector
{
    ...
    private void HandlePrivmsg(string line)
    {
        ...
        // эмоуты Twitch: "25:0-4,12-23/354:30-35" → токены :em_{id}
        if (tags.TryGetValue("emotes", out var em) && !string.IsNullOrEmpty(em))
        {
            var spans = new List<(int start, int end, string token)>();
            foreach (var group in em.Split('/'))
                foreach (var item in group.Split(','))
                {
                    ...
                    if (int.TryParse(item[(colon + 1)..dash], out var s) && int.TryParse(item[(dash + 1)..], out var e))
                        spans.Add((s, e, $":em_{item[..colon]}:"));
                }
            spans.Sort((a, b) => b.start.CompareTo(a.start));
            var sb = new StringBuilder(text);
            foreach (var (s, e, token) in spans)
                if (s >= 0 && e < sb.Length && e >= s) { sb.Remove(s, e - s + 1); sb.Insert(s, token); }
            text = sb.ToString();
        }
...
public sealed class KickConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            content = Regex.Replace(content, @"\[emote:(\d+):([\w]+)\]", ":emk_$1:");
...
public sealed class VkVideoLiveConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        await Net.WsSend(_ws, JsonSerializer.Serialize(new { subscribe = new { channel = $"public-chat:{wsChannel}" }, id = 2 }), ct);
        await Net.WsSend(_ws, JsonSerializer.Serialize(new { subscribe = new { channel = $"channel-info:{wsChannel}" }, id = 3 }), ct);
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            // Centrifugo может прислать несколько JSON-объектов, разделённых \n
            foreach (var line in frame.Split('\n', StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (s == "{}")
                {
                    await Net.WsSend(_ws, "{}", ct);   // pong
                    continue;
                }
                try { HandleFrame(s); }
                catch { /* неизвестная структура — пропускаем */ }
            }
        }
    }
    ...
    private void HandleFrame(string json)
    {
        ...
        if (!doc.RootElement.TryGetProperty("push", out var push)) return;
    ...
    {
        if (!data.TryGetProperty("data", out var msg)) return;
        ...
        if (msg.TryGetProperty("data", out var blocks) && blocks.ValueKind == JsonValueKind.Array)
        {
            foreach (var b in blocks.EnumerateArray())
            {
                ...
                switch (bt)
                {
                    ...
                    case "smile":
                        sb.Append(b.TryGetProperty("name", out var sn) ? $":{sn.GetString()}:" : "");
                        break;
                    case "mention":
                        sb.Append('@')
