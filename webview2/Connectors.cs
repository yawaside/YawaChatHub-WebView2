...
public sealed class TwitchEventSubConnector : IPlatformConnector
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        if (broadcasterId.Length == 0 || selfId.Length == 0) return;
        ...
        foreach (var (type, version, condition) in subs)
        {
            try
            {
                ...
                if (!res.IsSuccessStatusCode)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"[eventsub] {type}: {(int)res.StatusCode} {err}");
                }
            }
            catch (Exception ex) { System.Diagnostics.Debug.WriteLine($"[eventsub] {type}: {ex.Message}"); }
        }
    }
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(_token))
            throw new InvalidOperationException(
                "DonationAlerts: нужен OAuth access token приложения (Клиент → API → токен доступа)");
        _status("connecting", 0);

        using var http = Net.Http();
        // токен вставляют по-разному: с префиксом и без — нормализуем
