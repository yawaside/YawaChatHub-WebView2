...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        // Версионно-независимая настройка композиции: не проверяем номер Edge /
        // Chromium и не выбираем ветку по версии. Если конкретный рантайм знает
        // эти флаги — он направляет дочерний рендер в перенаправляемую поверхность,
        // которую видит цветовой ключ Win32; старые рантаймы безопасно игнорируют
        // неизвестный feature-name. Уже заданные пользователем аргументы сохраняем.
        const string overlayArgs =
            "--disable-features=RemoveRedirectionBitmap --disable-direct-composition " +
            "--disable-backgrounding-occluded-windows --disable-renderer-backgrounding";
        var existingArgs = Environment.GetEnvironmentVariable("WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS") ?? "";
        if (!existingArgs.Contains("RemoveRedirectionBitmap", StringComparison.OrdinalIgnoreCase))
            existingArgs = $"{existingArgs} {overlayArgs}".Trim();
        Environment.SetEnvironmentVariable("WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS", existingArgs);
