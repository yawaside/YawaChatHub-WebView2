...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        // Служебный WebView старого оверлея невидим, но должен продолжать
        // рисовать страницу для двух снимков (black/white alpha recovery).
        // Эти общие Chromium-флаги не привязаны к версии и не вмешиваются в
        // DirectComposition/оконные стили; пользовательские аргументы сохраняем.
        const string renderArgs =
            "--disable-backgrounding-occluded-windows --disable-renderer-backgrounding";
        var existingArgs = Environment.GetEnvironmentVariable("WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS") ?? "";
        if (!existingArgs.Contains("disable-renderer-backgrounding", StringComparison.OrdinalIgnoreCase))
            existingArgs = $"{existingArgs} {renderArgs}".Trim();
