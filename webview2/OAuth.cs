...
public static class AppAuth
{
    ...
    /// Адреса на наш слушатель = redirect_uri.
    /// Twitch требует литеральный loopback-IP; VK не принимает IP-адреса
    /// в redirect, но пропускает домен localhost (слушатель dual-stack,
    /// поэтому ответит и на 127.0.0.1, и на localhost/::1).
    private static string LoopbackRedirectUri(int port) => $"http://127.0.0.1:{port}/oauth";
    private static string LocalhostRedirectUri(int port) => $"http://localhost:{port}/oauth";

    private static string BuildAuthUrl(string platform, string state, int port) => platform switch
    {
        // Обе площадки идут напрямую на loopback: внешние страницы-мосты
        // (yawachathub.netlify.app и пр.) из цепочки исключены полностью.
        "twitch" =>
        ...
        "vkplay" =>
            ...
            $"&redirect_uri={Uri.EscapeDataString(LocalhostRedirectUri(port))}" +
            "&response_type=token" +
            "&scope=offline" +
    ...
    private sealed class LoopbackHttp : IDisposable
    {
        ...
        /// Первый свободный порт из списка; слушает loopback в dual-stack:
        /// принимает и http://127.0.0.1 (Twitch), и http://localhost (VK),
        /// независимо от того, как система разрешит "localhost" (127.0.0.1
        /// или ::1) — v4-only слушатель на "localhost→::1" молча молчал.
        public static LoopbackHttp Start()
        {
            foreach (var p in Ports)
            {
                try
                {
                    // dual-stack: IPv6Only=false — слушает и ::1, и 127.0.0.1.
                    // Конструктора (IP, port, family) не существует (CS1729) —
                    // адрес задаётся через IPEndPoint.
                    var l = new TcpListener(new IPEndPoint(IPAddress.IPv6Loopback, p));
                    l.Server.SetSocketOption(SocketOptionLevel.IPv6, SocketOptionName.IPv6Only, false);
                    l.Start();
                    return new LoopbackHttp(l, p);
                }
                catch { /* без IPv6 — пробуем IPv4 */ }
