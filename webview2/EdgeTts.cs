...
public sealed class EdgeTts : IDisposable
{
    ...
    public void Warmup()
    {
        _ = Task.Run(async () =>
        {
            ...
            // Прогреваем именно WebSocket-эндпоинт синтеза: первое соединение
            // тратит время на DNS + TLS, из-за чего первая фраза заметно
            // запаздывала. Соединение сразу закрываем — важен прогретый канал.
            try
            {
                ...
                await ws.ConnectAsync(
                    new Uri($"{WsBase}?TrustedClientToken={TrustedToken}" +
                            $"&Sec-MS-GEC={GenerateSecMsGec()}&Sec-MS-GEC-Version={GecVersion}" +
        ...
        CancellationToken outer = default)
    {
        ...
        if (!RussianVoices.Any(v => v.Id.Equals(voice, StringComparison.OrdinalIgnoreCase)))
            voice = DefaultVoice;
        ...
        try
        {
            ...
            // МГНОВЕННО отправляем аудио в OBS в ту же миллисекунду, как только синтез готов
            try { onAudioReady?.Invoke(audio); } catch { }

            // Параллельно запускаем воспроизведение на локальном ПК
