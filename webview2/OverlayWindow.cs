...
public sealed class OverlayWindow : Form
{
    ...
    private void DragTick()
    {
        ...
        // РЕЖИМ ИЗМЕНЕНИЯ РАЗМЕРА: страница сообщила, за какой край тянут
        if (_resizing)
        {
            if (held)
            {
                ...
                var scr = Screen.PrimaryScreen?.WorkingArea ?? new Rectangle(0, 0, 1280, 720);
                Size = new Size(
                    Math.Clamp(w, 240, scr.Width),
                    Math.Clamp(h, 90, scr.Height - 40));
            }
            else
            {
                _resizing = false;
                ...
                if (!(_freePosition && !_locked)) _dragTimer.Stop();
            }
            return;
        }
    ...
    {
        // Прозрачность сохраняется ВСЕГДА: в режиме перемещения окно НЕ
        // затемняется, а лишь показывает тонкую рамку, которую рисует
        // страница. Мышь перехватывать не нужно — перетаскивание идёт
        // опросом курсора глобально, поэтому chroma-key не отключаем.
        BackColor = Color.Black;
        TransparencyKey = Color.Black;
        Opacity = 1;
        _clickThrough = movable ? false : _clickThrough;
    ...
    /// <summary>
    /// Начать изменение размера окна за край. Направление: "e" (правый),
    /// "s" (нижний) или "se" (угол). Тянем тем же опросом мыши, что и
    /// перетаскивание, — WebView2 забирает себе оконные сообщения.
    /// </summary>
