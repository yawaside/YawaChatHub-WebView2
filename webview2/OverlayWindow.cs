...
public sealed class OverlayWindow : Form
{
    ...
    public OverlayWindow(HostApi api)
    {
        ...
        // фон самого контрола тоже ключевой: прозрачные пиксели страницы
        // показывают то, что под WebView2 — а там должен быть именно ключ
        _web.BackColor = KeyColor;
        // прозрачный фон WebView2 — ДО создания CoreWebView2 (иначе игнорируется);
        // страхует переменную окружения WEBVIEW2_DEFAULT_BACKGROUND_COLOR (Program.Main)
