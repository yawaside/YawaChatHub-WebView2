...
public sealed class OverlayWindow : Form
{
    ...
    {
        // ПРОЗРАЧНОСТЬ НИКОГДА НЕ ОТКЛЮЧАЕТСЯ.
        // Раньше на время перетаскивания окно делалось сплошным тёмным —
        // и, поскольку перетаскивание включено по умолчанию, подложка
        // не исчезала ни при каком стиле. Теперь chroma-key активен всегда,
        // а тянуть оверлей можно за видимую ручку (её рисует страница).
        BackColor = Color.Black;
        TransparencyKey = Color.Black;
        Opacity = 1;

        _clickThrough = !movable;   // в режиме перемещения окно ловит мышь
