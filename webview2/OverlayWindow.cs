...
///
/// ПРОЗРАЧНОСТЬ: цветовой ключ (TransparencyKey) с WebView2 ненадёжен —
/// DirectComposition-поверхность контрола ключ не пробивает, окно оставалось
/// непрозрачным (чёрный прямоугольник), а цветной ключ при этом делал
/// «дырявые» пиксели ещё и клик-сквозными. Поэтому используем DWM-стекло
/// (DwmExtendFrameIntoClientArea с -1): страница рисует только подложку rgba,
/// прозрачные пиксели показывают рабочий стол, текст остаётся чётким.
/// </summary>
public sealed class OverlayWindow : Form
{
    ...
    /// <summary>
    /// Стеклянная прозрачность вместо цветового ключа: DWM растягивает
    /// «стекло» на всё окно, а прозрачные пиксели DirectComposition WebView2
    /// показывают то, что под окном. Обязательно — иначе оверлей непрозрачный.
    /// </summary>
    protected override void OnHandleCreated(EventArgs e)
    {
        base.OnHandleCreated(e);
        var m = new MARGINS { Left = -1, Right = -1, Top = -1, Bottom = -1 };
        try { DwmExtendFrameIntoClientArea(Handle, ref m); } catch { }
    }
    ...
    private void UpdateExStyle()
    {
        ...
        // WS_EX_LAYERED не нужен (и мешает): прозрачность делает DWM-стекло,
        // а не цветовой ключ. WS_EX_TRANSPARENT — только в режиме сквозного клика.
