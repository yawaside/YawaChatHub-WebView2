...
public sealed class OverlayWindow : Form
{
    ...
    private void SafeDispatch(string type, object payload)
    {
        ...
        try { Dispatch(type, payload); } catch { /* одно битое событие не гасит оверлей */ }
    }
    ...
    private void Repaint()
    {
        ...
        catch { /* одна неудачная перерисовка не ломает оверлей */ }
        finally
        {
            _painting = false;
        }
    ...
    private void DrawScene(Graphics g, int w, int h)
    {
        ...
        int lineH = Math.Max(18, (int)(fs * 2.15));
        int statsH = _cfg.ShowStats ? Math.Max(16, (int)(fs * 1.9)) : 0;
        int y = pad;
        ...
        if (movable)
        {
            DrawDragHandle(g, pad, y, fs);
            y += Math.Max(18, fs + 8) + _cfg.Spacing;
        }

        var stats = VisibleStats();
        if (_cfg.StatsPos == "top" && stats != null)
        {
            DrawStatsBar(g, pad, y, w - pad * 2, statsH, stats);
            y += statsH + _cfg.Spacing;
        }
        ...
        if (list.Count == 0 && stats == null)
        {
            DrawHint(g, pad, y, fs);
            return;
        }
        if (_cfg.Growth == "up") list.Reverse();

        foreach (var m in list)
        {
            if (y + lineH > h - (stats != null && _cfg.StatsPos == "bottom" ? statsH + _cfg.Spacing : 0)) break;
            DrawMessage(g, m, pad, y, w - pad * 2, lineH);
            y += lineH + _cfg.Spacing;
        }

        if (_cfg.StatsPos == "bottom" && stats != null)
    ...
    private void DrawMessage(Graphics g, Msg m, int x, int y, int width, int height)
    {
        ...
        // акцентная полоска площадки слева
        if (_cfg.AccentBar)
        {
            var pc = Color.FromArgb(230, PlatformColor(m.Platform));
            FillRounded(g, new Rectangle(x + 2, y + 3, 3, height - 6), pc, 2);
        }

        int tx = x + 8;
        int ty = y + Math.Max(0, (height - (int)(fs * 1.6)) / 2);
        const int gap = 4;
        ...
        if (_cfg.ShowTime && m.Ts > 0)
        {
            ...
            tx += DrawSeg(g, t.ToString("HH:mm "), GetFont(fs, bold: false), ScaleAlpha(Color.FromArgb(255, 139, 145, 168), textA), tx, ty, fs) + gap;
        }
        // короткий тег площадки
        if (_cfg.ShowPlatform)
        {
            string tag = PlatformShort(m.Platform);
            var chipW = (int)(fs * 1.85);
            var chip = new Rectangle(tx, ty + Math.Max(0, (int)(fs * 0.12)), chipW, (int)(fs * 1.3));
            FillRounded(g, chip, ScaleAlpha(PlatformColor(m.Platform), textA), 4);
            TextOut(g, tag, GetFont((int)(fs * 0.78), bold: true), ScaleAlpha(Color.White, textA), chip, centered: true);
            tx += chipW + gap + 2;
        }
        // ник
        ...
        if (m.Amount.HasValue)
        {
            ...
            int aw = TextRenderer.MeasureText(amt, GetFont(fs, bold: true)).Width;
            var pill = new Rectangle(tx, ty + (int)(fs * 0.1), aw + 8, (int)(fs * 1.35));
            FillRounded(g, pill, ScaleAlpha(Color.FromArgb(255, 245, 158, 11), 70, textA), 4);
            TextOut(g, amt, GetFont(fs, bold: true), ScaleAlpha(Color.FromArgb(255, 253, 230, 138), textA), pill, centered: true);
            tx += aw + 10;
        }
        // текст
        if (!string.IsNullOrWhiteSpace(m.Text))
            DrawSeg(g, m.Text, GetFont(fs, bold: false), ScaleAlpha(Color.FromArgb(255, 238, 240, 248), textA), tx, ty, fs, width - (tx - x) - 4);
    }
    ...
    private void DrawStatsBar(Graphics g, int x, int y, int width, int height, List<PlatformStat> shown)
    {
        ...
        {
            var pc = PlatformColor(p.Platform);
            string seg = PlatformShort(p.Platform);
            int chipW = (int)(fs * 1.85);
            var chip = new Rectangle(tx, y + Math.Max(0, (height - (int)(fs * 1.3)) / 2), chipW, (int)(fs * 1.3));
            FillRounded(g, chip, ScaleAlpha(pc, textA), 4);
            TextOut(g, seg, GetFont((int)(fs * 0.78), bold: true), ScaleAlpha(Color.White, textA), chip, centered: true);
            tx += chipW + 3;
