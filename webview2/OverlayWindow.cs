...
public sealed class OverlayWindow : Form
{
    ...
    // после этих сообщений DWM на части систем на мгновение теряет ULW-кадр
    // (окно «мигает» прозрачным при клике) — на каждый из них пере-выкладываем
    private const int WM_MOUSEACTIVATE = 0x21, WM_ACTIVATE = 0x6, WM_NCACTIVATE = 0x86,
    ...
    public OverlayWindow(HostApi api)
    {
        ...
        // при появлении окна сразу возвращаем кадр и переснимаем страницу
        VisibleChanged += (_, _) =>
        {
            PresentFrame();
            ScheduleCapture();
    ...
    private async Task CaptureOnceAsync()
    {
        ...
        try
        {
            ...
            // Защита от «мигания»: если кадр пустой (WebView2 снёс в момент
            // композитинга/анимации), ДЕРЖИМ ПРЕДЫДУЩИЙ кадр, а не проваливаем
            // окно в прозрачное.
            bool hasContent = false;
            for (int i = 3; i < _pixels.Length; i += 4)
            {
                if (_pixels[i] > 0) { hasContent = true; break; }
            }
    ...
    protected override void WndProc(ref Message m)
    {
        ...
        // мышь/активация/изменение позиции — на всякий случай возвращаем кадр,
        // чтобы окно не просвечивало на мгновение при взаимодействии
        if (m.Msg is WM_MOUSEACTIVATE or WM_ACTIVATE or WM_NCACTIVATE
            or WM_WINDOWPOSCHANGED or WM_WINDOWPOSCHANGING)
            PresentFrame();
    }
