...
public sealed class OverlayWindow : Form
{
    ...
    public OverlayWindow(HostApi api)
    {
        ...
        // Цветовой ключ НЕ задаётся в конструкторе: layered-окно с ключом
        // пропускает мышь через все «прозрачные» пиксели на уровне ОС,
        // из-за чего оверлей нельзя было двигать. Ключ включается только
        // вместе со сквозным кликом (см. UpdateExStyle).
        Controls.Add(_web);
    ...
    /// Режим перемещения: страница показывает рамку, мышь ловится окном.
    ...
    /// Скрытый оверлей не получает сообщения — экономим CPU.
    public void Forward(string type, object payload)
    {
        if (!Visible) return;   // окно спрятано — рендер не нужен
    ...
    private void UpdateExStyle()
    {
        ...
        if (_clickThrough)
        {
            // СКВОЗНОЙ КЛИК: цветовой ключ делает прозрачные пиксели
            // «дырявыми» и для мыши — нажатия уходят в игру.
            TransparencyKey = Color.Black;
            SetLayeredWindowAttributes(Handle, (uint)ColorTranslator.ToWin32(Color.Black), 0, LWA_COLORKEY);
        }
        else
        {
            // ОБЫЧНЫЙ РЕЖИМ: без цветового ключа. Windows при ключе всегда
            // пропускает мышь через прозрачные пиксели — поэтому окно было
            // «не двигаемым». Здесь всё окно принимает мышь, а прозрачность
            // обеспечивает альфа-канал WebView2 (DWM-композиция).
