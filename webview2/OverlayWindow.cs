...
/// <summary>
/// Игровой оверлей: окно WebView2 поверх всех окон, прозрачное и «клик-сквозь»
/// (аналог electron-окна с setIgnoreMouseEvents). Страница — рендерер #/overlay.
///
/// ПРОЗРАЧНОСТЬ — прямой подход (capture-вариант опробован и снят:
/// CapturePreview/WebViewBrush отдают прозрачные пиксели ЧЁРНЫМИ — альфа от
/// движка к битмапу не доходит в принципе). WebView2 стоит прямо в окне,
/// прозрачность делает цветовой ключ: фон формы и контрола — УНИКАЛЬНЫЙ цвет
/// rgb(1,2,3), TransparencyKey вырезает его, страница рисует только контент.
///
/// Версионной проверки нет: Program.Main передаёт мягкие флаги композиции
/// (неизвестные старому движку флаги безопасно игнорируются), а само окно
/// каждую секунду смотрит ФАКТИЧЕСКИЕ стили текущих дочерних HWND WebView2.
/// Если поверхность реально помечена WS_EX_NOREDIRECTIONBITMAP, стиль снимается
/// на месте и включается перенаправляемая layered-поверхность. Поэтому решение
/// переживает старый/новый Evergreen Runtime и пересоздание render-процесса.
/// </summary>
public sealed class OverlayWindow : Form
{
    /// Уникальный цветовой ключ: почти чёрный, но не (0,0,0) — чтобы не
    /// пересекаться с пикселями тёмного рендера (premultiplied-transparent
    /// композится в чистый чёрный, и «круглый» чёрный ключ дырявил контент).
    ...
    /* Runtime-guard: WebView2 может пересоздать внутренние HWND после
       навигации или перезапуска render-процесса. Проверяем ФАКТИЧЕСКИЕ
       стили раз в секунду — никаких сравнений номера/версии рантайма. */
    private readonly System.Windows.Forms.Timer _compositionGuard = new() { Interval = 1000 };
    ...
    [DllImport("user32.dll")] private static extern bool EnumChildWindows(IntPtr hwnd, EnumWindowsProc callback, IntPtr lParam);
    ...
    private const uint LWA_COLORKEY = 0x1, LWA_ALPHA = 0x2;
    private const uint SWP_NOMOVE = 0x2, SWP_NOSIZE = 0x1, SWP_NOZORDER = 0x4, SWP_FRAMECHANGED = 0x20;
    private const int VK_LBUTTON = 0x01;

    /// <summary>
    /// Цветовой ключ с самого рождения: layered-стиль выдаём сразу,
    /// а не поздним SetWindowLong (переключение стиля на живом окне у части
    /// систем сбрасывает атрибуты прозрачности).
    /// </summary>
    ...
    public OverlayWindow(HostApi api)
    {
        ...
        // контента страницы) становятся прозрачными, в т.ч. для мыши
        AllowTransparency = true;
        BackColor = KeyColor;
        TransparencyKey = KeyColor;

        // фон самого контрола и движка = ТОЧНО цвет ключа. Задаём ДО
        // создания CoreWebView2 — не полагаемся на трактовку alpha=0
        // конкретной версией/реализацией композитора.
        _web.BackColor = KeyColor;
        _web.DefaultBackgroundColor = KeyColor;
        Controls.Add(_web);

        // ПЕРЕТАСКИВАНИЕ: WebView2 создаёт дочерние окна, которые забирают себе
        // все сообщения мыши — WM_NCHITTEST/HTCAPTION у формы не срабатывает.
        // Надёжное решение: опрашиваем позицию курсора и состояние ЛКМ напрямую.
        _dragTimer.Tick += (_, _) => DragTick();
        _compositionGuard.Tick += (_, _) => NormalizeWebViewComposition();
        ...
        Load += async (_, _) =>
        {
            ...
            // Фон движка = ТОЧНО цвет ключа (а не transparent, который разные
            // реализации композитора сводят то в чёрный, то в фон родителя).
            _web.DefaultBackgroundColor = KeyColor;

            // Цвет ключа рисуется самой HTML-страницей: это устраняет зависимость
            // от того, как конкретный WebView2 трактует transparent. !important
            // перекрывает общий тёмный фон index.html и эффекты React только в
            // отдельном окне #/overlay; видимый контент остаётся поверх ключа.
            await _web.CoreWebView2.AddScriptToExecuteOnDocumentCreatedAsync("""
                (() => {
                  const install = () => {
                    if (document.getElementById('yawa-overlay-key')) return;
                    const parent = document.head || document.documentElement;
                    if (!parent) return;
                    const s = document.createElement('style');
                    s.id = 'yawa-overlay-key';
                    s.textContent = 'html,body,#root,#boot{background:rgb(1,2,3)!important;background-color:rgb(1,2,3)!important}';
            ...
            NormalizeWebViewComposition();
            _compositionGuard.Start();

            // ВАЖНО: окно оверлея тоже общается с хостом (settings.get и пр.),
    ...
    /// Прозрачность текста/подложки — внутри страницы (rgba); окно вырезает
    /// фоновый цвет ключа, текст и подложка остаются такими, как нарисованы.
    /// </summary>
    public void Apply(JsonNode? cfg)
    {
        ...
        Opacity = 1; // текст не трогаем: полупрозрачность делает подложка в CSS
    ...
    /// <summary>
    /// Режим перемещения — только перетаскивание. Мышь над ВСЕМ окном
    /// пропускает стиль WS_EX_TRANSPARENT (настройка clickThrough / хоткей),
    /// а невидимые области пропускают её всегда — сами, цветовым ключом.
    /// </summary>
    ...
    /// Вернуть окно в центр оѾвного экрана.
    ...
    /// <summary>
    /// Нормализация композиции БЕЗ проверки версии WebView2.
    /// Обходим текущие дочерние HWND контрола. Только если конкретное окно
    /// реально имеет WS_EX_NOREDIRECTIONBITMAP, снимаем его и включаем
    /// полно-непрозрачный layered-режим дочерней поверхности: тогда пиксели
    /// снова проходят через поверхность родителя и его color-key.
    /// Старые/другие рантаймы, где такого стиля нет, вообще не затрагиваются.
    /// </summary>
    private void NormalizeWebViewComposition()
    {
        if (!_web.IsHandleCreated) return;

        void Fix(IntPtr hwnd)
        {
            if (hwnd == IntPtr.Zero) return;
            int ex = GetWindowLong(hwnd, GWL_EXSTYLE);
            if ((ex & WS_EX_NOREDIRECTIONBITMAP) == 0) return;

            int next = (ex & ~WS_EX_NOREDIRECTIONBITMAP) | WS_EX_LAYERED;
            SetWindowLong(hwnd, GWL_EXSTYLE, next);
            // Дочерняя поверхность остаётся полностью видимой; layered нужен
            // только для возвращения redirection bitmap родительскому окну.
        ...
        Fix(_web.Handle);
        EnumChildWindows(_web.Handle, (hwnd, _) =>
        {
            Fix(hwnd);
            return true;
        }, IntPtr.Zero);

        // Нормализация дочерних HWND не должна сбросить ключ родителя.
    ...
    protected override void Dispose(bool disposing)
    {
        if (disposing)
        {
            try { _compositionGuard.Dispose(); } catch { }
    ...
    private void UpdateExStyle()
    {
        ...
        // SetWindowLong сам по себе стили не применяет — нужен FRAMECHANGED.
        SetWindowPos(Handle, IntPtr.Zero, 0, 0, 0, 0,
            SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED);
        // Явно переустанавливаем ключ ПОСЛЕ смены стилей: смена layered-стиля
        // сбрасывает атрибуты окна, и TransparencyKey перестаёт действовать.
        SetLayeredWindowAttributes(Handle, (uint)ColorTranslator.ToWin32(KeyColor), 0, LWA_COLORKEY);
    }
}
...
public sealed class WidgetServer : IDisposable
{
    ...
    private async Task Serve(HttpListenerContext ctx)
    {
        ...
        try
        {
            ...
            {
                // Принудительно открываем виджет-роут и делаем холст прозрачным
                // ДО запуска React. !important перекрывает ранний тёмный фон из
                // index.html; это работает в OBS независимо от версии CEF.
                const string injection =
                    ...
                    "</style><script>location.hash='#/widget'+location.search</script>";
