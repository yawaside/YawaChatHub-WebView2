...
public static class TikTokLive
{
    ...
    private sealed class LiveReaderWindow : Form
    {
        ...
        public async Task StartAsync()
        {
            ...
            // БЛОКИРУЕМ скачивание и декодирование видео/аудио потоков TikTok:
            // нам нужен ТОЛЬКО текст чата. Блокировка медиа-потоков снижает
            // потребление памяти на 300+ МБ и убирает 90% нагрузки на CPU.
            ...
            {
                // Видео, звук, аватары, превью и web-шрифты чату не нужны.
                // Их блокировка сильно снижает RAM/CPU скрытой страницы.
                if (e.ResourceContext is CoreWebView2WebResourceContext.Media
        ...
        private const string ReaderScript = """
            (() => {
              ...
              // Помечаем все текущие сообщения как историю (чтобы не спамить при подключении)
              const markAllHistory = () => {
                try {
                  document.querySelectorAll(MESSAGE_NODES).forEach((n) => {
              ...
              // MutationObserver ловит только поступающие новые сообщения.
              // Пока история загружается пачками, ждём 1.5 сек тишины и
              // помечаем ВСЁ имеющееся как прочитанное. Лишь затем новые
              // одиночные узлы считаются реальными сообщениями.
              const observer = new MutationObserver((records) => {
                ...
                // Большая пачка после старта — тоже история/виртуализация DOM,
                // не отправляем её как десятки «новых» сообщений.
                if (added.length > 8) { markAllHistory(); return; }
                added.forEach(push);
              });

              // Удаляем видео-плеер и WebGL-канвас TikTok — это полностью убирает
              // нагрузку на CPU/GPU и предотвращает расход памяти.
              const dropMedia = () => {
                try {
                  document.querySelectorAll('video, audio').forEach((v) => {
                    try { v.pause(); v.src = ''; v.srcObject = null; v.remove(); } catch (e) {}
                  });
              ...
              // Страховка для тихого чата: если история не породила mutations,
              // через 6 секунд делаем снимок истории и включаем live-режим.
              setTimeout(() => {
                if (!liveReady) {
                  markAllHistory();
                  dropMedia();
