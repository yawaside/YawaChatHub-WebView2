    ...
    private sealed class LiveReaderWindow : Form
    {
        ...
        private const string ReaderScript = """
            (() => {
              ...
              // TikTok регулярно меняет вёрстку, поэтому ищем по нескольким
              // признакам сразу — от точных data-атрибутов до классов контейнеров.
              ...
              const state = () => {
                const el = viewerNode();
                const viewers = parseCount(el?.textContent || '');
                // эфир идёт, если найден чат ИЛИ счётчик зрителей, либо есть видео
                const live = !!findList() || !!el || !!document.querySelector('video');
                send({ t: 'state', live, v: viewers });
              };

              const tick = () => { attach(); state(); };
              document.addEventListener('DOMContentLoaded', tick);
              // первые секунды опрашиваем часто: страница догружается постепенно
              setInterval(tick, 2000);
              [300, 800, 1500, 3000, 5000, 8000].forEach((d) => setTimeout(tick, d));
            })();
            """;
    }
}
