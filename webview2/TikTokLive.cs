    ...
    private sealed class LiveReaderWindow : Form
    {
        ...
        private const string ReaderScript = """
            (() => {
              ...
              // SIGI_STATE может весить несколько МБ — парсим его ОДИН раз
              // и дальше держим только число зрителей. Иначе каждые 3 секунды
              // шел огромный JSON.parse, который и грузил процессор.
              let jsonViewers = -1;
              const readJsonViewersOnce = () => {
                if (jsonViewers >= 0) return jsonViewers;
                try {
                  ...
                  if (script && script.textContent) {
                    ...
                    jsonViewers = typeof uc === 'number' ? uc : 0;
                  }
                } catch (e) { jsonViewers = 0; }
                return jsonViewers;
              };
              ...
              const state = () => {
                let viewers = readJsonViewersOnce();
                const el = viewerNode();
