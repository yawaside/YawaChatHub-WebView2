            ...
            (() => {
              ...
              const observer = new MutationObserver((records) => {
                ...
                for (const r of records) {
                  // если сразу добавилась большая пачка (исторический дамп) — помечаем как историю
                  if (r.addedNodes.length > 8) {
                    r.addedNodes.forEach((n) => {
                      try {
                        const target = n.nodeType === 1 && n.matches && n.matches(MESSAGE_NODES) ? n : n.querySelector ? n.querySelector(MESSAGE_NODES) : null;
                        if (target) { const { nick, text } = extract(target); if (text) seen.add(nick + '|#|' + text); }
                      } catch (e) {}
                    });
                    continue;
                  }
                  r.addedNodes.forEach((n) => {
                    try {
                      if (n.nodeType === 1 && n.matches && n.matches(MESSAGE_NODES)) push(n);
                      else if (n.querySelector) n.querySelectorAll(MESSAGE_NODES).forEach(push);
                    } catch (e) {}
                  });
                }
              });
              ...
              // Через 4 секунды после старта вся история гарантированно помечена
              // и включается приём исключительно новых сообщений
              setTimeout(() => {
                markAllHistory();
                dropMedia();
                liveReady = true;
