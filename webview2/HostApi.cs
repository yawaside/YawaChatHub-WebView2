...
public sealed class HostApi : IDisposable
{
    ...
    private async Task<object?> RunModerationAsync(JsonNode req)
    {
        ...
        // 1) Валидируем токен и scope на КАЖДОЕ действие: старый/неполный
        // токен больше не даёт безымянный 401/403.
        using var vh = new HttpClient { Timeout = TimeSpan.FromSeconds(15) };
        vh.DefaultRequestHeaders.Add("Authorization", $"OAuth {token}");
        var validateRes = await vh.GetAsync("https://id.twitch.tv/oauth2/validate");
        var validateBody = await validateRes.Content.ReadAsStringAsync();
        if (!validateRes.IsSuccessStatusCode)
            throw new InvalidOperationException("Токен Twitch истёк — выйдите и войдите заново");

        using var validate = JsonDocument.Parse(validateBody);
        var modId = Net.FindString(validate.RootElement, "user_id") ?? "";
        var tokenLogin = (Net.FindString(validate.RootElement, "login") ?? "").ToLowerInvariant();
        var scopes = validate.RootElement.TryGetProperty("scopes", out var sa) && sa.ValueKind == JsonValueKind.Array
        ...
        string requiredScope = action switch
        {
            ...
            "mode-slow" or "mode-emote" or "mode-followers" or "mode-subs" => "moderator:manage:chat_settings",
            _ => "",
        };
        ...
        // 2) ID пользователя по login через Helix. Это надёжнее room-id/user-id
        // IRC и работает, даже если сообщение пришло до получения ROOMSTATE.
        async Task<string> UserIdByLogin(string login)
        {
            ...
            return Net.FindString(arr[0], "id") ?? "";
        }
