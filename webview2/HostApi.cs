...
public sealed class HostApi : IDisposable
{
    ...
    private async Task<object?> Route(string method, object?[] a)
    {
        switch (method)
        {
            ...
            /* ---------- озвучка (SAPI, раньше было в electron-коннекторах) ---------- */
            case "tts.speak":
            {
                var text = Str(a, 0) ?? "";
                var voice = Str(a, 1) ?? "";
                var rate = a.Length > 2 ? Int(a, 2) : 0;      // SAPI -10..10
            ...
            case "tts.clear": _tts.Clear(); return null;
    ...
    {
        _win.SendEvent("chat.message", message);
        _overlay?.Forward("chat.message", message);
        _widget.Broadcast(message);
    }
