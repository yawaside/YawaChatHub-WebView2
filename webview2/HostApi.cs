...
public sealed class HostApi : IDisposable
{
    ...
    public HostApi(MainWindow win, WebView2 web)
    {
        ...
        // пакетирование сообщений для оверлея и виджета
        _overlayFlush.Tick += FlushOverlayBatch;
    ...
    private async Task<object?> Route(string method, object?[] a)
    {
        switch (method)
        {
            ...
            case "tts.speak":
            {
                ...
                {
                    var mp3 = await _edge.SpeakAsync(text, voice, rate * 10, volume);
                    // Озвучка текста уходит в OBS Browser Source:
                    // при включённой опции «Управлять аудио через OBS» звук
                    // попадает в аудиомикшер OBS и звучит на стриме.
    ...
    /// </summary>
    /* ---------- пакетирование сообщений для оверлея и виджета ----------
       Раньше каждое сообщение сериализовалось и рассылалось отдельно:
       при плотном чате это десятки ре-рендеров в секунду. Теперь копим
       кадр (~90 мс) и отправляем ОДНИМ событием. */
    private readonly List<object> _overlayBatch = new();
    private readonly System.Windows.Forms.Timer _overlayFlush = new() { Interval = 90 };
    private readonly object _batchGate = new();

    public void PushChatMessage(object message)
    {
        // главное окно получает сообщения сразу: ему нужна мгновенная лента
        try { _win.SendEvent("chat.message", message); } catch { }

        lock (_batchGate) _overlayBatch.Add(message);
        if (!_overlayFlush.Enabled) _overlayFlush.Start();
    }

    /// Отправка накопленного пакета в оверлей и виджет (вызывается таймером).
    private void FlushOverlayBatch(object? sender, EventArgs e)
    {
        _overlayFlush.Stop();
        object[] batch;
        lock (_batchGate)
        {
            if (_overlayBatch.Count == 0) return;
            batch = _overlayBatch.ToArray();
            _overlayBatch.Clear();
        }
        // одна посылка вместо N: оверлей и виджет перерисовываются раз в кадр
