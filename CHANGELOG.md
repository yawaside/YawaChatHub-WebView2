...
### Исправлено
- Возвращено изменение размера безрамочного главного окна за края/углы через ручной `WM_NCHITTEST`
- Сообщения больше не дублируются при включении оверлея: один ChatMsg рассылается один раз каждому WebView, overlay работает в passive-режиме
- OverlayWindow получил собственные команды drag/hide; перемещается мышью даже без рамки, если не включены locked/clickThrough
- Изменения оформления оверлея применяются без пересоздания/закрытия окна; сохранение bounds не запускает SettingsChanged на каждый пиксель
- Kick переведён с несуществующего IRC на публичный Pusher WebSocket; VK Video Live — на public REST chat polling; исправлен YouTube renderer и polling публичной live_chat страницы
