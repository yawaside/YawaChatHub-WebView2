...
**Как устроен portable exe:** фронтенд (`dist/index.html` из Vite) **встраивается
в exe** как ресурс и при первом запуске распаковывается в
`%LOCALAPPDATA%\YawaChatHub\web`, откуда его отдаёт WebView2 через
`SetVirtualHostNameToFolderMapping`. Благодаря этому один файл `YawaChatHub.exe`
работает автономно — никаких папок рядом с ним не требуется.

**Если окно пустое:** приложение больше не остаётся «пустым и неубиваемым» —
показывается экран запуска с рабочей кнопкой закрытия, а при сбое панель
диагностики с кнопками «Установить WebView2», «Повторить» и «Закрыть».
Требуется компонент **Microsoft Edge WebView2 Runtime** (в Windows 11
предустановлен; для Windows 10 ставится по кнопке в окне ошибки).
...
> рендер) для этого не подходит. Поэтому workflow делает две сборки:
> `npm run build` для веб-версии и `npx vite build` для desktop/WebView2.
> После этого `desktop/`-шаги собирают `YawaChatHub.exe` и NSIS-установщик.
