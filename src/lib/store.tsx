...
interface AppCtxValue {
  ...
  saving: boolean;
  demo: boolean;
  /** true только в реальном C# WebView2 portable EXE — без симулятора. */
  desktop: boolean;

  messages: ChatMsg[];
  clearFeed: () => void;

  /** Ключ "platform:channel" → состояние коннектора/стримера. */
  channelStatus: Record<string, ChannelState>;
...
export function AppProvider({
  ...
  children: ReactNode;
  /** Лендинг: локальный интерактивный мок. */
  demo?: boolean;
  /** Portable WebView2: только реальные сообщения от C# connector-слоя. */
  desktop?: boolean;
  /** Оверлей только отображает события: без повторного TTS/публикации. */
  passive?: boolean;
}) {
  const [settings, setSettings] = useState<Settings>(DEFAULT_SETTINGS);
  const [loaded, setLoaded] = useState(false);
  const [messages, setMessages] = useState<ChatMsg[]>([]);
  const [channelStatus, setChannelStatus] = useState<Record<string, ChannelState>>({});
  ...
  useEffect(() => {
    ...
    if (desktop) {
      // В native EXE нет HTTP API и нет mock-данных: настройки читаются
      // напрямую из SettingsService.cs через COM host object `sp`.
      void (async () => {
        const ready = await waitForDesktopBridge();
        const nativeSettings = ready ? desktopSettings() : null;
        if (!alive) return;
        if (nativeSettings) setSettings(deepMerge(DEFAULT_SETTINGS, nativeSettings));
        setLoaded(true);
      })();
      return () => { alive = false; };
    }
  ...
  const patch = useCallback(
    (p: Record<string, unknown>) => {
      ...
      if (desktop) {
        // Немедленный settings.patch() на диск, без сетевого API и без mock.
        desktopPatch(p);
        return;
      }
  ...
  const publishMsg = useCallback(
    (msg: ChatMsg) => {
      // Защита от повторной доставки одного сообщения при переподключении
      // bridge/overlay. Один id появляется в конкретной ленте только один раз.
      if (seenMessages.current.has(msg.id)) return;
      seenMessages.current.add(msg.id);
      if (seenMessages.current.size > 500) {
        const keep = Array.from(seenMessages.current).slice(-250);
        seenMessages.current = new Set(keep);
      }
      setMessages((m) => {
      ...
      if (s.tts.enabled && filterRef.current) {
        ...
        if (res.ok) {
          ...
          // Desktop: только SAPI5 из TtsService.cs. Браузерная речь здесь
          // принципиально не включается.
          if (desktop) desktopTtsSpeak(speech);
          else {
            engineRef.current?.enqueue(speech);
          }
        }
      }
      if (!demo && !desktop) {
    ...
    [demo, desktop, passive],
  );

  // ── Демо-коннекторы ────────────────────────────────────────────────────
  useEffect(() => {
    // КРИТИЧНО: portable desktop не запускает ChatSimulator ни при каких
    // условиях. В него попадают только sp:chat от C# реальных коннекторов.
  ...
  }, [loaded, desktop, publishMsg]);

  // ── Реальные события C# connector-слоя ─────────────────────────────────
  useEffect(() => {
    if (!desktop) return;
    const offChat = onDesktopEvent<ChatMsg>("sp:chat", (msg) => {
      // Показываем только сообщения, которые прислал native connector.
      // C# не отправляет здесь демо-заглушки.
      if (msg?.id && msg.platform && !msg.sys) publishMsg(msg);
    });
    const offSettings = onDesktopEvent<Settings>("sp:settings", (next) => {
      if (next) setSettings((s) => deepMerge(s, next));
    });
    const offClients = onDesktopEvent<number>("sp:widget:clients", (n) =>
      setWidgetClients(Number(n) || 0),
    );
    const offHotkey = onDesktopEvent<string>("sp:hotkey", (action) => {
      if (action === "tts:skip") desktopTtsSkip();
      if (action === "tts:clear") desktopTtsStopAll();
    });
    const offStatus = onDesktopEvent<{ platform: string; channel: string; status: string }>(
      "sp:channel:status",
      (s) => {
        if (!s?.platform || !s.channel) return;
        const states: ChannelState[] = ["connecting", "online", "offline", "unsupported"];
        const st = states.includes(s.status as ChannelState) ? (s.status as ChannelState) : "connecting";
        const key = `${s.platform}:${s.channel}`;
        setChannelStatus((prev) => ({ ...prev, [key]: st }));
      },
    );
    return () => { offChat(); offSettings(); offClients(); offHotkey(); offStatus(); };
  }, [desktop, publishMsg]);

  // ── число OBS-клиентов (SSE для веб-версии) ─────────────────────────────
  useEffect(() => {
  ...
  const addChannel = useCallback(
    (platform: PlatformId, raw: string): string | null => {
      ...
      patch({ channels });
      // В native-режиме сразу запускаем реальный C# connector. Демо-всплески
      // допустимы только в браузерных режимах, никогда в portable EXE.
      if (desktop) desktopAddChannel(platform, v.value);
      else simRef.current?.burst(3);
      toast(`${platformById(platform).name}: канал ${v.value} добавлен`);
      return null;
    },
    [desktop, patch, toast],
  );
  ...
  const removeChannel = useCallback(
    (platform: PlatformId, channelId: string) => {
      ...
      if (desktop) desktopRemoveChannel(platform, channelId);
  ...
  const ttsToggle = useCallback(() => {
    ...
    if (!next) {
      if (desktop) desktopTtsStopAll();
      else engineRef.current?.stopAll();
    }
  ...
  const ttsSkip = useCallback(() => {
    if (desktop) desktopTtsSkip();
    else engineRef.current?.skip();
  ...
  const speakTest = useCallback(
    (text?: string) => {
      ...
      if (desktop) desktopTtsSpeak(speech);
      else engineRef.current?.enqueue(speech);
  ...
  const value = useMemo<AppCtxValue>(
    () => ({
      settings, loaded, patch, saving, demo, desktop,
      messages, clearFeed, channelStatus,
    ...
    [settings, loaded, patch, saving, demo, desktop, messages, clearFeed, channelStatus, addChannel, removeChannel,
