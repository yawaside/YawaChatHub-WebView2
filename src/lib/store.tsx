...
interface AppCtxValue {
  ...
  /** Ключ "platform:channel" → реально ли коннектор подключён. */
...
export function AppProvider({
  ...
  useEffect(() => {
    ...
    const offStatus = onDesktopEvent<{ platform: string; channel: string; connected: boolean }>(
      "sp:channel:status",
      (s) => {
        if (!s?.platform || !s.channel) return;
        const key = `${s.platform}:${s.channel}`;
        setChannelStatus((prev) => ({ ...prev, [key]: !!s.connected }));
      },
    );
