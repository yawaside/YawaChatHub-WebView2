...
export function AppProvider({
  ...
  useEffect(() => {
    ...
    const offHotkey = onDesktopEvent<string>("sp:hotkey", (action) => {
      // Событие зарегистрированного WinAPI hotkey обрабатывает уже существующий
      // документный handler ниже — преобразуем его в CustomEvent не нужно.
      if (action === "tts:skip") desktopTtsSkip();
      if (action === "tts:clear") desktopTtsStopAll();
    });
    return () => { offChat(); offSettings(); offClients(); offHotkey(); };
  }, [desktop, publishMsg]);
  ...
  const value = useMemo<AppCtxValue>(
    ...
    [settings, loaded, patch, saving, demo, desktop, messages, clearFeed, addChannel, removeChannel,
