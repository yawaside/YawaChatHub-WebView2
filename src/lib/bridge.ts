...
function call<T = unknown>(method: string, ...args: unknown[]): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    // Озвучка завершается после проигрывания фразы, поэтому ей нужен отдельный
    // таймаут. Старые 5 секунд преждевременно выбрасывали фразу из очереди.
    const timeout = method === "tts.speak" ? 120_000 : 8_000;
    window.setTimeout(() => {
      if (pending.has(id)) {
        pending.delete(id);
