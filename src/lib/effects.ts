...
export function textOutline(width: number | undefined, fallbackShadow = false): string {
  ...
  const steps = w <= 2 ? 8 : 12;
  for (let i = 0; i < steps; i++) {
    const angle = (Math.PI * 2 * i) / steps;
    const x = +(Math.cos(angle) * w).toFixed(2);
    const y = +(Math.sin(angle) * w).toFixed(2);
    shadows.push(`${x}px ${y}px 0 rgba(0,0,0,0.95)`);
  }
  // мягкая подложка под текстом — добавляет читаемости на светлой картинке
  shadows.push(`0 ${Math.max(1, Math.round(w / 2))}px ${w * 2}px rgba(0,0,0,0.85)`);
  return shadows.join(", ");
}
