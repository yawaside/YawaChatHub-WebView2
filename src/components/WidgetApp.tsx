      ...
      es.onmessage = (ev) => {
        try {
          ...
          if (d.event === "chat.message" && d.payload) {
            ...
            // Озвучка текста от приложения: проигрываем MP3 внутри Browser Source,
            // чтобы OBS захватывал звук в свой микшер
            const p = d.payload as { audio?: string; volume?: number };
            if (p?.audio) {
              try {
                const a = new Audio(`data:audio/mp3;base64,${p.audio}`);
                a.volume = Math.min(1, Math.max(0, p.volume ?? 1));
                void a.play().catch(() => {});
              } catch { }
            }
          }
