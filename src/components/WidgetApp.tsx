...
export default function WidgetApp() {
  ...
  useEffect(() => {
    // Холст OBS Browser Source ВСЕГДА прозрачный. Настройка c.transparent
    // управляет только карточками сообщений ниже — она не должна заливать
    // весь источник сплошным чёрным/светлым прямоугольником.
  ...
  useEffect(() => {
    const push = (raw: Partial<ChatMsg>) => {
      ...
      // события показываем только если включено настройкой
      if (kind !== "chat" && kind !== "system" && !cfgRef.current.showEvents) return;
  ...
  return (
    <div
      ...
      style={{
        ...
        gap: c.spacing,
        // корень занимает всю ширину Browser Source и всегда прозрачен;
        // фон выбранного стиля рисуется только на отдельных сообщениях
