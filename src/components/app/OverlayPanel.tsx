...
import { Box, CheckRow, Chip, Slider, ToggleRow } from "./ui";
    ...
    <div className="grid grid-cols-1 gap-3 overflow-y-auto xl:grid-cols-2">
      ...
      <Box title="Подложка и читаемость" icon={<Blend size={14} style={{ color: "var(--dw-dim)" }} />}>
        <div className="flex flex-col gap-3">
          ...
          {/* живой предпросмотр */}
          <div className="mt-1 rounded-xl p-3" style={{ background: "linear-gradient(120deg,#1f6feb33,#8b5cf633), var(--dw-panel2)" }}>
            <p className="pb-1.5 text-[10px] uppercase" style={{ color: "var(--dw-dim)" }}>
              предпросмотр
            </p>
            <div
              className="flex items-baseline gap-1.5 px-2.5 py-1.5"
              style={{
                background: `rgba(8,10,16,${cfg.backdropOpacity})`,
                borderRadius: cfg.radius,
                borderLeft: cfg.accentBar ? "3px solid #9146FF" : "none",
                opacity: cfg.textOpacity,
                color: "#fff",
                fontSize: cfg.fontSize,
                textShadow: cfg.shadowText ? "0 1px 3px rgba(0,0,0,0.95)" : "none",
              }}
            >
              <PlatformIcon id="twitch" size={Math.round(cfg.fontSize * 0.85)} />
              <b style={{ color: "#FF6B6B" }}>nekto_san</b>
              <span>текст остаётся чётким при любой подложке</span>
            </div>
          </div>
        </div>
      </Box>
