...
import { OVERLAY_STYLES, styleName } from "../../lib/presets";
import { StyleGallery } from "./Presets";
import { prettyCombo } from "./HotkeysPanel";
...
export default function OverlayPanel({ cfg, patch, toast, hotkey }: Props) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Collapsible title="Стиль оформления" hint={styleName(OVERLAY_STYLES, cfg.styleId)} defaultOpen>
      ...
      <Collapsible title="Окно оверлея" hint={cfg.freePosition ? (cfg.locked ? "зафиксировано" : "режим перемещения") : "привязка к углу"}>
        <div className="flex flex-col gap-2">
          <ToggleRow
            label="Свободное размещение"
            hint="в режиме перемещения v1.2.4 временно делает фон видимым; после фиксации прозрачность возвращается"
            checked={cfg.freePosition}
            onChange={(v) => {
              apply({ ...cfg, freePosition: v, corner: v ? "free" : "top-left", locked: false });
              toast(v ? "Переместите оверлей и зафиксируйте позицию" : "Включена прозрачная привязка к углу");
            }}
          />
          ...
          {cfg.freePosition ? (
            <>
              <div className="flex gap-1.5">
                <button
                  onClick={() => {
                    apply({ ...cfg, locked: !cfg.locked });
                    toast(cfg.locked ? "Оверлей разблокирован" : "Позиция зафиксирована — прозрачность возвращена");
                  }}
              ...
              <p className="text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                После перемещения нажмите «Зафиксировать» — старый прозрачный режим v1.2.4 включится снова.
              </p>
            </>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-1.5">
                {CORNERS.map((corner) => (
                  <Chip key={corner.id} active={cfg.corner === corner.id} onClick={() => apply({ ...cfg, corner: corner.id })}>
                    {corner.label}
                  </Chip>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="mb-1 block text-[11px]" style={{ color: "var(--dw-dim)" }}>Отступ X</label>
                  <Slider value={cfg.offsetX} min={0} max={400} step={2} onChange={(v) => apply({ ...cfg, offsetX: v })} format={(v) => `${v}px`} />
                </div>
                <div>
                  <label className="mb-1 block text-[11px]" style={{ color: "var(--dw-dim)" }}>Отступ Y</label>
                  <Slider value={cfg.offsetY} min={0} max={400} step={2} onChange={(v) => apply({ ...cfg, offsetY: v })} format={(v) => `${v}px`} />
                </div>
              </div>
            </>
          )}
          ...
          <ToggleRow
            label="Сквозной клик"
            hint="нажатия мыши проходят сквозь оверлей в игру; по умолчанию выключен, включается хоткеем"
      ...
      <Collapsible title="Содержимое и поведение" hint={cfg.showEvents ? "чат и события" : "только чат"}>
        <div className="flex flex-col gap-1">
          ...
          <ToggleRow
            ...
            hint="донаты, сабы, рейды в общем потоке"
            checked={cfg.showEvents}
            onChange={(v) => apply({ ...cfg, showEvents: v })}
          />
