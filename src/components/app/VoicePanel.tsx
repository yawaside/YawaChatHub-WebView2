...
import { Box, IconBtn, Slider, ToggleRow } from "./ui";
...
export default function VoicePanel({ tts, patch, queue, paused, onPauseToggle, onSkip, onClearQueue, onTest }: Props) {
  ...
  const ruVoices = useMemo(() => {
    const ru = voices.filter((v) => v.lang.toLowerCase().startsWith("ru"));
    return ru.length ? ru : voices;
  }, [voices]);

  const selected = tts.voice || ruVoices[0]?.name || "";
  useEffect(() => {
    if (!tts.voice && ruVoices[0]) patch({ voice: ruVoices[0].name });
  ...
  return (
    <div className="grid grid-cols-1 gap-3 overflow-y-auto xl:grid-cols-2">
      {/* голос и параметры */}
      <Box title="Голос и параметры" icon={<Volume2 size={14} style={{ color: "var(--dw-dim)" }} />}>
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            >
              {ruVoices.length === 0 && <option value="">Голос по умолчанию</option>}
              {ruVoices.map((v) => (
                <option key={v.name} value={v.name} style={{ color: "#000" }}>
                  {v.name} ({v.lang})
                </option>
              ))}
            </select>
