...
import { isPresetActive } from "../../lib/presets";
          ...
          >
            ...
            >
              <span
                ...
                style={{ color: p.swatch[0], fontFamily: p.previewFont }}
              >
                Аа Bb
              </span>
            ...
            <p
              ...
              style={{ color: on ? "var(--dw-accent-2)" : "var(--dw-text)", fontFamily: p.previewFont }}
            >
              {p.name}
            </p>
...
/* ---------------- демо-сообщения для предпросмотров ---------------- */
...
export function FeedPreview({ view }: { view: ChatViewConfig }) {
  ...
  return (
    <PreviewFrame title="Предпросмотр ленты">
      <div
        className="flex flex-col"
        style={{ gap: view.spacing ?? 6, fontSize: view.fontSize, fontFamily: fontStack(view.fontFamily) }}
      >
        {SAMPLE.slice(0, 4).map((m, i) => (
          <div
            ...
            style={{
              ...
              borderRadius: view.density === "tight" ? 0 : 10,
              background: view.backdrop
                ? m.kind === "donate"
                  ? "color-mix(in srgb, #f59e0b 12%, transparent)"
                  : "var(--dw-panel)"
                : "transparent",
              borderLeft: m.kind === "donate" ? "2px solid #f59e0b" : "2px solid transparent",
            }}
          >
            {view.showTime && (
            ...
            {view.showBadges && i === 0 && (
              <span className="shrink-0 rounded px-1 font-bold uppercase" style={{ fontSize: "0.6em", background: "rgba(52,211,153,0.2)", color: "#34d399" }}>
                mod
              </span>
            )}
            <span
            ...
            {m.amount && (
              <span className="shrink-0 rounded px-1 font-bold" style={{ background: "#f59e0b22", color: "#fbbf24" }}>
                {m.amount} ₽
              </span>
            )}
            <span style={{ color: "var(--dw-text)" }}>{m.text}</span>
          </div>
        ))}
      </div>
    </PreviewFrame>
...
export function WidgetPreview({ cfg }: { cfg: WidgetConfig }) {
  ...
  return (
    <PreviewFrame title="Предпросмотр виджета OBS" scene>
      <div
        className="flex flex-col justify-end rounded-lg p-2"
        style={{
          fontSize: Math.min(cfg.fontSize, 20),
      ...
      >
        {shown.map((m, i) => (
          <div
            key={i}
            className="flex items-baseline gap-1.5 rounded-lg px-2 py-1 leading-snug"
            style={{
          ...
          >
            <span className="flex shrink-0 translate-y-[1px]">
              <PlatformIcon id={m.platform} size={Math.round(Math.min(cfg.fontSize, 20) * 0.85)} />
            </span>
            <span className="shrink-0 font-bold" style={{ color: nickColor(m.author, platformMeta(m.platform).color) }}>
              {m.author}
            </span>
      ...
      <p className="pt-1.5 text-[10px]" style={{ color: "var(--dw-dim)" }}>
        Шахматный фон — прозрачность в OBS. Показано {shown.length} из {cfg.max} строк.
      </p>
    </PreviewFrame>
  );
}
...
export function OverlayPreview({ cfg }: { cfg: OverlayConfig }) {
  const backdrop = {
    background: `rgba(8,10,16,${cfg.backdropOpacity})`,
    borderRadius: cfg.radius,
  };
  const textShadow = cfg.shadowText ? "0 1px 3px rgba(0,0,0,0.95), 0 0 2px rgba(0,0,0,0.9)" : "none";
  const list = (cfg.eventsOnly ? SAMPLE.filter((s) => s.kind === "donate") : SAMPLE).slice(
    0,
  ...
  const statsBar = cfg.showStats ? (
    <div
      ...
      style={{ ...backdrop, color: "#fff", textShadow, opacity: cfg.textOpacity }}
    >
      {cfg.showDonations && (
  ...
  return (
    <PreviewFrame title="Предпросмотр оверлея" scene>
      <div
        className="flex flex-col"
        style={{
          gap: cfg.spacing ?? 4,
      ...
      >
        ...
        {list.map((m, i) => (
          <div
            key={i}
            className="flex items-baseline gap-1.5 px-2 py-1 leading-snug"
            style={{
              ...backdrop,
              color: "#fff",
              textShadow,
          ...
          >
            {cfg.showTime && <span style={{ fontSize: "0.78em", opacity: 0.7 }}>12:34</span>}
            {cfg.showPlatform && (
              <span className="flex shrink-0 translate-y-[1px]">
...
function PreviewFrame({
  ...
  return (
    <div className="rounded-xl border p-2.5" style={{ background: "var(--dw-panel)", borderColor: "var(--dw-line)" }}>
      <p className="pb-2 text-[10px] font-bold tracking-[0.14em] uppercase" style={{ color: "var(--dw-dim)" }}>
        {title}
      </p>
      <div
