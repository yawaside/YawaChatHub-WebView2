...
export function BotAuthCards({ toast }: { toast: (t: string) => void }) {
  ...
  const login = async (t: AuthTarget) => {
    ...
    setBusy(t.id);
    toast(`Открыли браузер — войдите в аккаунт бота ${t.name}`);
    try {
      ...
      // Ошибка redirect — это настройки консоли площадки, а не сбой приложения:
      //   Twitch → http://127.0.0.1:8123/oauth (и 8124–8126)
      //   VK     → http://localhost:8123/oauth (и 8124–8126)
      const hint = /security error|invalid_request|redirect/i.test(msg)
        ? t.id === "vkplay"
          ? " В настройках приложения VK добавьте разрешённый redirect URI http://localhost:8123/oauth (и 8124–8126)."
          : " В настройках приложения Twitch добавьте redirect URI http://127.0.0.1:8123/oauth (и 8124–8126)."
        : "";
