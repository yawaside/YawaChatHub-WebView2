...
export function BotAuthCards({ toast }: { toast: (t: string) => void }) {
  ...
  const login = async (t: AuthTarget) => {
    ...
    try {
      ...
      // «Security Error» от VK — это НЕ наша ошибка: в настройках VK-приложения
      // не разрешён loopback redirect URI http://127.0.0.1:8123/oauth (и 8124–8126)
      const hint = /security error|invalid_request|redirect/i.test(msg)
        ? " В настройках приложения площадки добавьте разрешённый redirect URI http://127.0.0.1:8123/oauth (и 8124–8126)."
        : "";
