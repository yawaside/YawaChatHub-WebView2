  ...
  const login = async (t: AuthTarget) => {
    ...
    toast(
      t.id === "vkplay"
        ...
        : `Открыли браузер — войдите в аккаунт бота ${t.name}`
    );
    try {
      ...
      // Twitch возвращается на локальный адрес — он должен быть в консоли Twitch.
      const hint =
        t.id === "twitch" && /redirect|invalid|отказано|подключени/i.test(msg)
          ? " В консоли Twitch добавьте OAuth Redirect URL http://127.0.0.1:8123/oauth (и 8124–8126)."
          : "";
