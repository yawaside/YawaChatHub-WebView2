...
import { Box, CheckRow, Slider } from "./ui";
...
export default function WidgetPanel({ cfg, patch, toast }: Props) {
  ...
  const url = `${info.url}?max=${cfg.max}&fs=${cfg.fontSize}&theme=${cfg.theme}${cfg.transparent ? "&transparent=1" : ""}`;
      ...
      <Box title="Параметры вывода">
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <Slider value={cfg.fontSize} min={11} max={28} step={1} onChange={(v) => patch({ ...cfg, fontSize: v })} format={(v) => `${v}px`} />
          </div>
