...
export default function ChatBotPanel({ toast }: { toast: (t: string) => void }) {
  const [commands, setCommands] = usePersisted<BotCommand[]>("botCommands", DEFAULT_COMMANDS);
  // Параметры VK ID хранятся локально и читаются хостом при входе
  const [vkClientId, setVkClientId] = usePersisted<string>("vkClientId", "");
  const [vkServiceToken, setVkServiceToken] = usePersisted<string>("vkServiceToken", "");
    ...
    <div className="flex h-full flex-col gap-3 overflow-y-auto">
      ...
      >
        ...
        <div className="pt-2.5">
          ...
          <TextInput
            value={vkClientId}
            onChange={(e) => setVkClientId(e.target.value.replace(/\D/g, ""))}
            placeholder="ID из настроек приложения VK ID"
          />
          <label className="mt-2 mb-1 block text-[11px]" style={{ color: "var(--dw-dim)" }}>
            Сервисный ключ VK ID (только для конфиденциального приложения)
          </label>
          <TextInput
            type="password"
            value={vkServiceToken}
            onChange={(e) => setVkServiceToken(e.target.value.trim())}
            placeholder="оставьте пустым для публичного приложения"
          />
          <label className="mt-2 mb-1 block text-[11px]" style={{ color: "var(--dw-dim)" }}>
          ...
          <p className="pt-1 text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
            В VK ID укажите: базовый домен localhost, доверенный Redirect URL http://localhost.
            Аккаунт VK ID и канал Live связаны не всегда — тогда укажите канал здесь.
          </p>
        </div>
