...
interface Props {
  ...
  collapsed: boolean;
  style?: ChannelsStyle;
  /** сумма донатов за сессию — показывается вместо зрителей у DonationAlerts */
            ...
            {isDA && active && (
              <>
                ...
                <span className="font-bold tabular-nums" style={{ color: "#FFB454" }}>
                  {(donationTotal ?? 0).toLocaleString("ru-RU")} {c.currency ?? "₽"}
                </span>
              </>
            )}
            {/* зрителей показываем только если эфир реально подтверждён */}
            {!isDA && live && c.viewers > 0 && (
              <>
                <span style={{ color: "var(--dw-dim)" }}>·</span>
...
export function AddChannelModal({
  ...
  const submit = () => {
    ...
    // TikTok: cookie повышает надёжность чата (webcast без неё часто молчит)
    onConnect(
      platform,
      normalizeUsername(raw),
      platform === "tiktok" && cookie.trim() ? { token: cookie.trim() } : undefined
    );
    onClose();
  };
              ...
              {platform === "tiktok" && (
                <div className="pt-1">
                  <label className="mb-1 block text-[11px]" style={{ color: "var(--dw-dim)" }}>
                    Cookie из браузера (если чат не приходит)
                  </label>
                  <TextInput
                    value={cookie}
                  ...
                  <p className="pt-1 text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
                    TikTok часто отдаёт чат только авторизованному браузеру. Скопируйте строку Cookie
                    со страницы эфира (DevTools → Network → заголовок Cookie).
                  </p>
                </div>
              )}
            </div>
