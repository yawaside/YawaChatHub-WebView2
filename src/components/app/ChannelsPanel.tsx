  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      >
        ...
        {/* мини-полоса площадок с иконками */}
        <div className="flex items-center gap-1 px-2.5 pb-2">
          {PLATFORMS.map((pl) => {
            const chans = channels.filter((c) => c.platform === pl.id);
            const live = chans.some((c) => c.status === "online");
            return (
              <span
                key={pl.id}
                title={`${pl.name}: ${chans.length ? `${chans.length} кан.` : "не подключено"}`}
                className="flex h-5 w-5 items-center justify-center transition-all"
                style={{
                  borderRadius: mono ? 3 : 6,
                  background: live ? `${pl.color}26` : "transparent",
                  opacity: chans.length ? 1 : 0.25,
                  boxShadow: live ? `0 0 0 1px ${pl.color}66` : "none",
                }}
              >
                <PlatformIcon id={pl.id} size={11} />
              </span>
            );
          })}
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pr-0.5">
        {channels.length === 0 ? (
          ...
          PLATFORMS.map((pl) => {
            ...
            return (
              <div key={pl.id}>
                {/* заголовок группы: только иконка площадки + счётчик, без названия */}
                <div className="flex items-center gap-1.5 px-1.5 pb-1" title={pl.name}>
                  <span
                    className="flex h-4.5 w-4.5 items-center justify-center"
                    style={{ borderRadius: mono ? 2 : 5, background: `${pl.color}22`, width: 18, height: 18 }}
                  >
                    <PlatformIcon id={pl.id} size={11} />
                  </span>
                  <span className="h-px flex-1" style={{ background: "var(--dw-line)" }} />
                  <span className="text-[9.5px] tabular-nums" style={{ color: "var(--dw-dim)" }}>
                    {chans.filter((c) => c.status === "online").length}/{chans.length}
                  </span>
                </div>
