...
import { CHANNEL_STATE_META, PLATFORMS, platformById, type ChannelState, type HotkeyAction, type PlatformId } from "@/lib/types";
...
function ChannelsPanel() {
  const { settings, patch, addChannel, removeChannel, channelStatus, desktop } = useApp();
    ...
    <div className="panel flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {settings.channels.map((c) => {
          ...
          const state: ChannelState = channelStatus[`${c.platform}:${c.channelId}`] ?? (desktop ? "connecting" : "online");
          const meta = CHANNEL_STATE_META[state];
          return (
            ...
            >
              <span
                className={"size-2 flex-none rounded-full" + (meta.pulse ? " live-dot" : "")}
                style={{ background: meta.color, boxShadow: state === "online" ? "0 0 6px var(--ok)" : "none" }}
                title={meta.label}
              />
              <span className="min-w-0 flex-1 truncate text-sm font-medium">{c.channelId}</span>
              <span className="flex-none text-[10px] font-semibold" style={{ color: meta.color }}>{meta.label}</span>
...
function MainView() {
  const { settings, patch, ttsToggle, channelStatus, desktop } = useApp();
  const collapsed = settings.channelsCollapsed;
  // В web-версии каналы эмулируются — «онлайн»; в desktop — «подключение…»,
  // пока реальный коннектор не даст ответ.
  const fallback: ChannelState = desktop ? "connecting" : "online";
      ...
      <div className="flex h-full min-h-0 gap-3">
        <aside className="panel flex w-12 flex-none flex-col items-center gap-2 py-3">
          ...
          <div className="flex flex-col items-center gap-1.5">
            {PLATFORMS.map((p) => {
              const list = settings.channels.filter((c) => c.platform === p.id);
              const states = list.map((c) => channelStatus[`${c.platform}:${c.channelId}`] ?? fallback);
              const dotColor = states.includes("online") ? "var(--ok)"
                : states.includes("connecting") ? "var(--warn)"
                : states.length > 0 ? "var(--danger)"
                : "var(--muted)";
              const dotLabel = states.includes("online") ? "онлайн"
                : states.includes("connecting") ? "подключение…"
                : states.includes("unsupported") ? "нет коннектора"
                : "офлайн";
              return (
                <span
                  key={p.id}
                  ...
                  title={`${p.name}${list.length ? `: ${list.length} канал(ов), ${dotLabel}` : ""}`}
                >
                  {p.short}
                  {list.length > 0 && (
                    <span
                      className={"absolute -right-1 -top-1 size-2.5 rounded-full" + (states.includes("online") || states.includes("connecting") ? " live-dot" : "")}
                      style={{
                        background: dotColor,
                        boxShadow: states.includes("online") ? "0 0 6px var(--ok)" : "none",
                      }}
                      title={dotLabel}
                    />
                  )}
