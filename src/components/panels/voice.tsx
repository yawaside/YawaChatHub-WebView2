...
import { desktopTtsVoices } from "@/lib/desktop-bridge";
...
export function VoicePanel() {
  const { settings, patch, ttsQueue, ttsPaused, ttsPauseToggle, ttsSkip, ttsClear, speakTest, toast, desktop } = useApp();
  ...
  useEffect(() => {
    if (desktop) {
      const native = desktopTtsVoices();
      setVoices(native.map((name) => ({ name, voiceURI: name, lang: "ru-RU" })));
      return;
    }
