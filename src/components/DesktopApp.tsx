...
import { isDesktop, onHostEvent, ttsBridge, windowCtl, applyHotkeys, channelsApi, overlayBridge, runModerationOnPlatform, botApi } from "../lib/bridge";
  ...
  useEffect(() => {
    ...
    return onHostEvent((type, payload) => {
      if (type === "chat.message") {
        ...
        const b = payload as { platform: PlatformId; account: string };
        if (b?.account) {
          setBotAuth((prev) => ({ ...prev, [b.platform]: b.account }));
          const exists = channelsRef.current.some(
            (c) => c.platform === b.platform && c.username.toLowerCase() === b.account.toLowerCase()
          );
          if (!exists) connectChannel(b.platform, b.account);
        }
