...
export default function DesktopApp() {
  ...
  const [botCommands] = usePersisted<BotCommand[]>("botCommands", DEFAULT_COMMANDS);
  ...
  const runBotCommand = useCallback(
    (msg: ChatMsg) => {
      ...
      // не отвечаем сами себе: иначе ответ-команда зациклится
      const self = botAuthRef.current[msg.platform];
      if (self && self.toLowerCase() === msg.author.toLowerCase()) return;
      ...
      if (!cmd) {
        // Диагностика: раньше несовпадение триггера проходило молча,
        // и понять, почему бот не ответил, было невозможно.
        const known = list.filter((c) => c.enabled).map((c) => norm(c.trigger));
        setFeed((f) =>
          [
            ...f,
            makeSystem(
              known.length === 0
                ? "Бот: команды не заданы — добавьте их в разделе «Чат-бот»"
                : `Бот: команда «${trigger}» не найдена (есть: ${known.join(", ")})`
            ),
          ].slice(-600)
        );
        return;
      ...
      const target =
        ...
        "";
      if (!target) {
        setFeed((f) => [...f, makeSystem("Бот: неизвестен канал для ответа")].slice(-600));
        return;
      }
      void sendChatMessage(msg.platform, target, answer)
        .then(() => {
          // ответ реально ушёл в чат площадки — показываем его и в ленте
          setFeed((f) => [...f, makeSystem(`Бот → ${target}: ${answer}`)].slice(-600));
        })
        .catch((e: unknown) => {
  ...
  const pushToFeed = useCallback(
    (msg: ChatMsg) => {
      ...
      feedFlushRef.current = window.setTimeout(() => {
        ...
        setFeed((f) => (f.length + batch.length > 600 ? [...f, ...batch].slice(-600) : [...f, ...batch]));
      }, 120);
    },
    []
  );
