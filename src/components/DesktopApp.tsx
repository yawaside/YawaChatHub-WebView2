...
export default function DesktopApp() {
  ...
  /* миграция: сквозной клик по умолчанию ВЫКЛЮЧЕН.
     В старых сборках он мог остаться сохранённым как true —
     один раз сбрасываем; дальше режим включается только хоткеем/панелью. */
  useEffect(() => {
    if (!desktop || !overlayLoaded) return;
    void settingGet<boolean>("overlayClickThroughMigrated", false).then((done) => {
      if (done) return;
      void settingSet("overlayClickThroughMigrated", true);
      setOverlayCfg((prev) => (prev.clickThrough ? { ...prev, clickThrough: false } : prev));
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
