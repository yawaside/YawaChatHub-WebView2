...
import { isDesktop, onHostEvent, settingGet, settingSet, ttsBridge, windowCtl, applyHotkeys, channelsApi, overlayBridge } from "../lib/bridge";
...
export default function DesktopApp() {
  ...
  const [overlayCfg, setOverlayCfg, overlayLoaded] = usePersisted<OverlayConfig>("overlay", DEFAULT_OVERLAY);
    ...
    (action: string) => {
      switch (action) {
        ...
        case "toggleClickThrough":
          // сквозной клик включается ТОЛЬКО этим хоткеем (или тумблером в настройках)
          setOverlayCfg((prev) => {
            const next = !prev.clickThrough;
            toast(next ? "Сквозной клик оверлея включён" : "Сквозной клик оверлея выключен");
  ...
  /* Одноразовый возврат рабочего оконного режима v1.2.4.
     Последующие версии сохраняли freePosition=true; старый OverlayWindow в
     режиме перемещения намеренно отключает chroma-key — из-за этого пользователь
     видел непрозрачный прямоугольник. Возвращаем привязку к углу; сквозной клик
     остаётся выключенным и включается только настройкой/хоткеем. */
  useEffect(() => {
    if (!desktop || !overlayLoaded) return;
    void settingGet<boolean>("overlayV124ModeRestored", false).then((done) => {
      if (done) return;
      void settingSet("overlayV124ModeRestored", true);
      setOverlayCfg((prev) => ({
        ...prev,
        clickThrough: false,
        freePosition: false,
