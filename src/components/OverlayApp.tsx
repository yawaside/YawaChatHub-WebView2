...
export default function OverlayApp() {
  ...
  useEffect(() => {
    const unsub = onHostEvent((type, payload) => {
      if (type === "chat.message") {
        ...
        // события показываем только если включено соответствующей настройкой
        const isEvent = m.kind !== "chat" && m.kind !== "system";
        if (isEvent && !cur.showEvents) return;
        setItems((p) =>
          [
            ...p,
            {
  ...
  /* ---------- перетаскивание окна мышью ----------
     При включённом сквозном клике окно мышь не ловит вообще —
     перетаскивание в этом режиме невозможно по определению. */
  useEffect(() => {
    if (!c.freePosition || c.locked || c.clickThrough) return;
  ...
  }, [c.freePosition, c.locked, c.clickThrough]);
  ...
  return (
    ...
    >
      ...
      {/* Ручка перетаскивания: единственный непрозрачный элемент, за который
          можно схватить окно, даже когда подложка полностью прозрачна. */}
      {movable && (
        <div
          className="flex items-center gap-1.5 self-start rounded-lg px-2 py-1 text-[10px] font-semibold whitespace-nowrap"
          style={{
