...
// Корневой Chromium-документ никогда не прокручивается: скролл принадлежит
// только ленте. Это также убирает белую горизонтальную полосу WebView2.
...
function DesktopOverlay() {
  ...
  return (
    <main
      className="h-screen w-screen overflow-hidden p-2"
      style={{ background: "transparent", cursor: !cfg.locked && !cfg.clickThrough ? "move" : "default" }}
      onMouseDown={(e) => {
        if (e.button !== 0 || cfg.locked || cfg.clickThrough) return;
        ...
        {cfg.showBorder && (
          <div
            ...
            style={{
              ...
              cursor: cfg.locked ? "default" : "move",
            }}
          >
            ...
            <span className="app-no-drag ml-auto flex items-center gap-1">
              ...
              <button type="button" className="opacity-50 transition-opacity hover:opacity-100" onClick={() => nativeWindow("hide")} aria-label="Скрыть">
                <X size={12} />
              </button>
            </span>
          </div>
...
function Root() {
  ...
  if (hash.includes("overlay")) {
    return (
      <AppProvider desktop passive>
        <DesktopOverlay />
      </AppProvider>
    );
  }
  // В portable EXE запрещён demo-режим: лента получает только sp:chat от
  // нативных C#-коннекторов реальных площадок.
